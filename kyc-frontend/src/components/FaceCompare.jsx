export default function FaceCompare({
  ...
  const normalisedFaceDecision = String(faceDecision || '').toUpperCase()
  const isHardFail =
    shouldRejectAsDifferentPerson ||
    normalisedFaceDecision === 'NO_MATCH' ||
    normalisedFaceDecision === 'SPOOF_FAIL'
  const needsReview =
    !isHardFail &&
    (
      normalisedFaceDecision === 'REVIEW' ||
      normalisedFaceDecision === 'RECAPTURE' ||
      faceUncertain ||
      verificationPassed === false
    )

  const color = isHardFail ? 'text-red-600' : needsReview ? 'text-amber-600' : 'text-emerald-600'
  const bgCls = isHardFail
    ? 'bg-red-50 border-red-200 text-red-700'
    : needsReview
      ? 'bg-amber-50 border-amber-200 text-amber-700'
      : 'bg-emerald-50 border-emerald-200 text-emerald-700'
  const statusText = isHardFail
    ? 'Match Failed'
    : normalisedFaceDecision === 'RECAPTURE'
      ? 'Recapture Needed'
      : needsReview
  ...
  return (
    <div className="w-full">
      <div className="mb-4 flex items-center justify-center gap-6">
        <div className="flex flex-col items-center gap-2">
          <img
            ...
            alt="ID"
            className="h-36 w-36 rounded-2xl border-2 border-[var(--warm-border)] object-cover shadow-sm"
          />
      ...
      <div className={`rounded-xl border px-4 py-2.5 text-center text-sm font-semibold ${bgCls}`}>
        {statusText}
      </div>
